...
VAE 用 encoder 学近似后验 $q(z|x)$，用 decoder 学生成分布 $p(x|z)$。训练时最大化 ELBO：要能从 $z$ 重建 $x$，又用 KL 把 $q(z|x)$ 拉向标准正态先验，让潜空间更连续、可采样。因此在潜空间里随机采样就能生成新图片，并且出现部分语义的聚类，但是相比于ddpm更加模糊，或者说是平滑。

（源代码没有留存，下文的code是ai写的，测试blog的code功能，仅供参考）
